...
const CVForm: React.FC<ParentFormBr> = ({ setShowForm }) => {
  const { register, handleSubmit, setValue, reset, formState: { errors }, control } = useForm<RowAcctgEntry>();
  const [rows, setRows] = useState<RowAcctgDetails[]>([{ accountLabel: "", acctnumber: "", debit: "", credit: "" }]);
  const { coaDataAccount, fetchCoaDataTable } = useCoa();
  const { createGV, loading } = useGeneralVoucher();
  ...
  const addRow = () => {
    setRows([...rows, { accountLabel: "", acctnumber: "", debit: "", credit: "" }]);
  };
