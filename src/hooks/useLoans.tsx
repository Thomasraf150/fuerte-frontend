...
const useLoans = () => {
  ...
  const fetchLoansForPagination = useCallback(async (
    ...
    try {
      ...
      console.log('[useLoans] GraphQL query variables:', variables);
      ...
      console.log('[useLoans] GraphQL response received:', {
      ...
      if (response.errors) {
        console.error('[useLoans] GraphQL errors:', response.errors);
        throw new Error(response.errors[0]?.message || 'GraphQL query failed');
      }

      // Ensure response.data exists
      if (!response.data || !response.data.getLoans) {
        console.error('[useLoans] Invalid response structure:', response);
      ...
      console.log('[useLoans] Fetch successful:', {
      ...
      console.error('[useLoans] fetchLoansForPagination error:', error);
      throw error;
    }
  }, [BORROWER_LOAN_QUERY]);
  ...
  const fetchSingLoans = async (loan_id: number) => {
    console.log('[useLoans] fetchSingLoans called with ID:', loan_id);
    setLoading(true);
    try {
      const { GET_AUTH_TOKEN } = useAuthStore.getState();
      const token = GET_AUTH_TOKEN();
      ...
      if (!token) {
        console.error('[useLoans] No auth token available');
        toast.error('Authentication required. Please log in again.');
        setLoading(false);
        return;
      }

      console.log('[useLoans] Fetching single loan, token available:', !!token);
      console.log('[useLoans] GraphQL query:', BORROWER_SINGLE_LOAN_QUERY);
      console.log('[useLoans] GraphQL variables:', { loan_id });
      ...
      console.log('[useLoans] Single loan response:', {
        hasErrors: !!response.errors,
        hasData: !!response.data,
      ...
      if (response.errors) {
        console.error('[useLoans] GraphQL errors in fetchSingLoans:', response.errors);
        throw new Error(response.errors[0]?.message || 'Failed to fetch loan');
      }

      if (!response.data?.getLoan) {
        console.error('[useLoans] No loan data in response:', response);
        throw new Error('Loan not found');
      }

      console.log('[useLoans] Setting loan single data:', response.data.getLoan);
      setLoanSingleData(response.data.getLoan);
    } catch (error: any) {
      console.error('[useLoans] fetchSingLoans error:', {
