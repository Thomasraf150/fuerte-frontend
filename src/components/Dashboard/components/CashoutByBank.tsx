...
interface CashoutByBankItem {
  bank_name: string;
  loan_count: string;
  total_net_cashout: string;
  total_pn_balance: string;
  total_ob?: string;
  total_collection: string;
  total_udi_collection: string;
}

interface SummaryTixData {
  cashout_by_bank: CashoutByBankItem[];
}

interface CashoutByBankProps {
  sumTixData: SummaryTixData | null;
  startDate: Date | undefined;
  endDate: Date | undefined;
}

const CashoutByBank: React.FC<CashoutByBankProps> = ({ sumTixData }) => {
  return (
    <div>
        ...
        <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default bg-gray-200 dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          <div className="overflow-x-auto">
            <table className="w-full table-auto mb-4 min-w-[800px]">
              ...
              <tbody>
                {sumTixData?.cashout_by_bank.map((item: CashoutByBankItem, i: number) => (
                  <tr key={i}>
                    ...
                    <td className="border-b text-right border-[#eee] px-4 py-3 dark:border-strokedark">
                      {'\u20B1'} {formatNumberComma(parseFloat(item.total_ob ?? item.total_pn_balance ?? '0'))}
                    </td>
                    <td className="border-b text-right border-[#eee] px-4 py-3 dark:border-strokedark">
